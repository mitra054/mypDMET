...
class QCsolvers:
    def __init__(self, solver, twoS=0, is_KROHF=False, e_shift=None, nroots=1, state_percent=None, verbose=0, memory=4000):
        ...
        if self.mol.spin == 0 and not self._is_KROHF:        
            ...
            self.mf = scf.ROHF(self.mol).density_fit()    
...
#####################################        
    def HF(self):
        ...
        self.mf.with_df._cderi = self.TEI
...
##################################           
    def RCCSD(self):
        ...
        # print("The CC energy is",self.cc.RCCSD(self.mf).run().e_corr)        
        # Ecorr, t1, t2 = self.cc.kernel(t1=t1_0, t2=t2_0)
        Ecorr, t1, t2 = self.cc.kernel(t1=t1_0, t2=t2_0)
        ECCSD = Ecorr + self.mf.e_tot
        self.t1 = t1
        self.t2 = t2
        if self.cc.converged: print('           CONGRATULATIONS: The solver has converged') 
...
##################################           
    def RCCSD_T_slow(self):
        ...
        self.mol.nelectron = self.Nel
        self.mf.__init__(self.mol, only_dfj=False, df = df.DF(self.mol))
        self.mf.get_hcore = lambda *args: FOCKcopy
        self.mf.get_ovlp = lambda *args: np.eye(self.Norb)
        self.mf.with_df._cderi = self.TEI
        self.mf.scf(self.DMguess)       
...
#########################################          
    def CASCI(self, solver = 'FCI', nevpt2_roots=None, nevpt2_nroots=10):
        ...
        if self.nroots == 1:
            ...
            casdm1_mo = self.mc.fcisolver.make_rdm1(self.mc.ci, cas_norb, self.mc.nelecas) #in CAS(MO) space
            ...
            for i, civec in enumerate(fcivec):
                ...
                core_MO = self.mc.mo_coeff[:,:core_norb]
                active_MO = self.mc.mo_coeff[:,core_norb:core_norb+cas_norb] 
                casdm1_mo = self.mc.fcisolver.make_rdm1(civec, cas_norb, self.mc.nelecas) #in CAS(MO) space
...
#########################################  
    ...
    def CASSCF(self, solver='FCI', state_specific_=None, state_average_=None, state_average_mix_=None, nevpt2_roots=None, cell=None, mc_dup=None, nevpt2_nroots=10, nevpt2_spin=0.0, kmf=None, w90=None, emb_orbs=None, ao2eo=None, mask4Gamma=None, OEH_type=None, emb_core_orbs = None, core_orbs=None):
        '''
        ...
        self.mf.with_df._cderi = self.TEI #not same in two different calculations; which basis is this in?
        self.mf.scf(self.DMguess)      
        DMloc = np.dot(np.dot(self.mf.mo_coeff, np.diag(self.mf.mo_occ)), self.mf.mo_coeff.T)
